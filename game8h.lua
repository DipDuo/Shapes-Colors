...
function scene:create (event)
	...
	--background
	local background = display.newImage( "Images/BkgdPurple.png", true )
...
-------------------------------------------------------------------------------------------------------------
	...
	function spawn ( event )
		...
		if image_group.numChildren >= 12 then
		...
		spawnObj.y =math.random(display.contentHeight)                             -- creates spawn objects -50 off screen
		spawnObj.rotation = ( 360 )
		spawnObj.index = frameIndex

		physics.addBody( spawnObj, "dynamic", { density = 3,
												friction = 1,
												bounce = .85,
												} )
	
		spawnObj:applyLinearImpulse( math.random(1,60), math.random(1,60), spawnObj.x, spawnObj.y )
		...
		function spawnObj:tap(self, event)  --adding tap event to remove spawned objects
	        ...
	        if isvalid then
	        	...
	        	if myData.settings.objTapped==8 then
					myData.settings.objTapped=0
	        		timer.performWithDelay(500, function() composer.gotoScene("nextlevel8h", "fade", 500); end)
	        	end	
	        else
	        	if myData.settings.soundOn == true then
	        		audio.play (wrong)
