		...
		function spawn ( self, event )
			...
			function spawnObj:tap(self, event)  --adding tap event to remove spawned objects
				...
				if isvalid then
					...
					if myData.soundOn == true then
						audio.play(correct)
					end
					if myData.settings.objTapped==2 then
						myData.settings.objTapped=0
						timer.performWithDelay(500, function() composer.gotoScene("nextlevel2e", "fade", 500); end)
					end
				else
