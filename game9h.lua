...
function scene:create (event)
	...
	--background
	local background = display.newImage( "Images/BkgdRed.png", true )
	...
	function spawn ( event )
		...
		function spawnObj:tap(self, event)  --adding tap event to remove spawned objects
	        ...
	        if isvalid then
	        	...
	        	if myData.settings.objTapped==9 then
					myData.settings.objTapped=0
	        		timer.performWithDelay(500, function() composer.gotoScene("nextlevel9h", "fade", 500); end)
	        	end	
	        else
	        	if myData.settings.soundOn == true then
	        		audio.play (wrong)
