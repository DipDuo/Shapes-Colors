...
-------------------------------------------------------------------------------------------------------------
	...
	function spawn ( event )
		...
		function spawnObj:tap(self, event)--adding touch event to remove spawned objects
			...
			if spawnObj.index == _G.frameIndexes[1] then
	        	...
	        	timer.performWithDelay(500, function() composer.gotoScene("nextlevel", "fade", 500); end)
	        else
	        	if myData.settings.soundOn == true then
	        		audio.play(wrong)
	        	end
	...
	spawnTimer=timer.performWithDelay( 205, spawn, 0 )-- determines the spawn rate of objects	
------------------------------------------------------------------------------------------------------------------
