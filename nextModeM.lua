...
local function handleButtonEvent( event )
    ...
    if ( "ended" == event.phase ) then
        composer.removeScene( "match1", false )
        composer.gotoScene( "match1", { effect = "crossFade", time = 333 } )
    end
end
...
function scene:create( event )
    ...
    masteredText = display.newText("You've mastered these levels!", display.contentCenterX, 50, native.systemFontBold, 20)
    masteredText:setFillColor(1,0,1)
    sceneGroup:insert(masteredText)


    nextLevelText = display.newText("Now try skill level: " .. myData.settings.currentMode2, display.contentCenterX, display.contentCenterY, native.systemFontBold, 20)
