...
function scene:create(event)
	...
	local function nextScene1 (easyLevel)
       ...
       composer.gotoScene("game5", options) --the scene/level that tap function transitions to
    end
...
function scene:show( event )
    ...
    if event.phase == "did" then
        composer.removeScene( "game4" ) -- change string value to reflect game level that was just finished
    end
end
